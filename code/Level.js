...
var Level = class Level {
  constructor (plan) {
  // read the "plan" and split it by new line characters
  //
    let rows = plan.trim().split('\n').map(l => [...l]) // this map function adds 3 periods to the start of the line of text. to save space on the plan, and make the level more visually appealing
    this.height = rows.length
    this.width = rows[0].length
    this.startActors = [] // initializes an empty array of actors
...
var State = class State {
  ...
  get player () { // returns actor if it is the player
...
/// ///  EVERYTHING ABOVE THIS LINE HAS BEEN PUT IN SEPARATE CLASSES AS WELL - except monster
...
State.prototype.update = function (time, keys) {
  ...
  if (this.level.touches(player.pos, player.size, 'lava')||this.level.touches(player.pos, player.size, 'monster')) {
...
Player.prototype.update = function (time, state, keys) {
  ...
  if (keys.ArrowDown && keys.ArrowUp && (ySpeed > 0)) {
    ySpeed -= jumpSpeed
  }
...
event handler and interrupting or resuming the animation whenever the Esc key is hit.
...
const pauseKey = trackKeys(['Escape'])
function runAnimation (frameFunc) {
  let lastTime = null
  function frame (time) {
    if (lastTime != null) {
      let timeStep = Math.min(time - lastTime, 100) / 1000
      // if (pauseKey.Escape){frameFunc(timeStep) = true} //this speeds up the frame rate when esc is held down
...
function runLevel (level, Display) {
  ...
  return new Promise(resolve => {
    runAnimation(time => {
	  ...
	  if (pauseKey.Escape) { // this needs work done to allow the game to pause and unpause, rather than just stopping dead
    return false
